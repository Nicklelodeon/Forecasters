...
class MLGenerateData6:
    ...
    def logic_normal(self, start_inventory, s_DC1, S_DC1, s_DC2, S_DC2, s_r1, S_r1, mean, std):
        ...
        for x in range(100):
            ...
            self.df = self.update_df(self.df, [demand, [start_inventory, s_DC1, S_DC1, s_DC2, S_DC2, s_r1, S_r1], self.state.total_sum()])
    ...
    def create_data(self):
        ...
        for i in data.columns[1:]:
            ...
            #     self.df = self.update_df(self.df, log1)
                ...
                for x in range(50):
                    log2 = self.logic_normal(random.randint(round(mean * 5), round(mean * 8)), random.randint(round(mean * 2), round(mean * 4)), random.randint(round(mean * 5), round(mean * 8)), random.randint(round(mean * 2), round(mean * 4)), random.randint(round(mean * 5), round(mean * 8)), random.randint(round(mean * 2), round(mean * 4)), random.randint(round(mean * 5), round(mean * 8)), mean, std)
                    if log2 is not None:
...
data.df.to_csv("/Users/nicholas/Documents/Misc/internship A*STAR/Work/6_24months_US_car_data.csv")
